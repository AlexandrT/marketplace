...
describe Marketplace::Bot::Parsers::ListParser do
  ...
  context "get_ids from page with more than 1000" do
    ...
    before do
      producer.should_receive(:load_list).with(0, 500).and_return(:producer)
      producer.should_receive(:load_list).with(501, 1000).and_return(:producer)
    end
  ...
  context "get_ids from page without arrow" do 
    ...
    it "check call method" do 
      # expect(list_parser.get_ids(page_without_arrow, start_price, end_price)).to eq(:producer)
